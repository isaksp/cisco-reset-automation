...
def Read_from_port(ser,rommon_event ,image_event, prompt_event,multilayer_event , switch_break_event, l2l3_old_event, flash_init_event, read_only_prompt_event):
    log_file = open("serial.log", "a")
    line_buffer = ""
    while ser.is_open == True:
        if ser.in_waiting > 0:
            ...
            for line in lines[:-1]:
                ...
                if "rommon" in line:rommon_event.set()

                #Checks when break should be sent to router
                if "Rom image verified correctly" in line:image_event.set()

                #Checks when break should be sent to Switch
                if "Send break" in line:switch_break_event.set()
                
                #ML Switch? 
                if "(interrupted)" in line:multilayer_event.set()
                
                #Answears the yes no prompt
                if "Press RETURN to get started!" in line:prompt_event.set()
                
                #black l2/l3 switch
                if "The system has been interrupted" in line: l2l3_old_event.set()

                #Initiazling flash on black switches l2/l3
                if "...done Initializing Flash." in line: flash_init_event.set()
                
                #Checks if read only
                if "read only file system" in line: read_only_prompt_event.set()
...
threading.Thread(target=Read_from_port, args=(ser, rommon_reached, image_reached, prompt_reached, multilayer_reached, switch_break_reached, l2l3_old_reached, flash_init_reached, read_only_prompt_reached), daemon=True).start()
...
if switch_break_reached.is_set():
    ...
    time_now = time.time()
    while True:
        if time.time() - time_now > timeout_time:
            ser.write(bytes(str(f"reset\r"), 'ISO-8859-1'))
            time.sleep(1)
            ser.write(bytes(str(f"y\r"), 'ISO-8859-1'))
            break
        elif read_only_prompt_reached.is_set():
            time.sleep(1)
            ser.write(bytes(str(f"SWITCH_IGNORE_STARTUP_CFG=1"), 'ISO-8859-1\r'))
            time.sleep(1)
            ser.write(bytes(str(f"\r"), 'ISO-8859-1\r'))
...
while True:
    ...
    if prompt_reached.is_set() and multilayer_reached.is_set() or prompt_reached.is_set() and l2l3_old_reached.is_set() and read_only_prompt_reached.is_set():
        ...
        ser.write(bytes(str(f"en\r"), 'ISO-8859-1'))
        ser.write(bytes(str(f"erase startup-config\r"), 'ISO-8859-1'))
        ser.write(bytes(str(f"yes\r"), 'ISO-8859-1'))
